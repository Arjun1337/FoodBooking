...
(function () {
    ...
    /*@ngNoInject*/
    function LoginController($state, $rootScope, AuthService, appConstant, TokenRestangular) {
        var vm = this;
        vm.loginError = "";
            ...
            if (isValid) {
                ...
                AuthService.authorize(data).then(function (response) {
                    ...
                    AuthService.login().then(function (response) {
                        ...
                        if (!$state.previous || $state.previous.url == "^"){
                            $state.go("main.home");
                        }
                        $state.go($state.previous);
                        // if($rootScope.currentUser.profile_type == "restaurant"){
                        //     $state.go('main.profile');
                        // }else if ($rootScope.currentUser.profile_type == "client"){
