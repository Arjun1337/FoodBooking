...
(function () {
    ...
    /*@ngNoInject*/
    function ClientDashboardController($rootScope,$scope,$timeout,ClientService, $state, $translate, $interval, $filter) {
        var vm = this;
        ...
        function isDayandBonusQuestion() {
            ...
            **/
            ...
            // alert(delayQuiz + "  " + isNextStep);
            // percentage = vm.quiz_prize.percentage;
            percentage = vm.quizClient.daily_percentage_discount;
            sumprice = vm.sum_price.sum_price;
            alert(vm.quizClient.percentage_step + " " + percentage);
            // alert(sumprice);
            ...
            if(vm.isCanDayquestion == true) {       // isDelay = true, isNextStep = true.
                vm.strDayQuestion = "available";
            } else if(isNextStep == false){         // isDelay = true || false, isNextStep = false.
                vm.strDayQuestion = "unavailable";
            } else {                                // isDelay = false, isNextStep = true.
                var remainHours = vm.quizSetting.quiz_delay_hrs - delayQuiz;
                var nextTime = new Date(date1);
                nextTime.setHours(date1.getHours() + vm.quizSetting.quiz_delay_hrs);
                nextTime = $filter('date')(nextTime, 'medium');
                vm.strDayQuestion = "available in " + remainHours + " hours, " + nextTime;
            }
        ...
        function getSetting(){
            ...
            if(vm.currentLanguage == "en")  currentLang = "ENG"; 
            else if(vm.currentLanguage == "cs")  currentLang = "CZE"; 
            // alert(vm.currentLanguage);
            var response = {
                "language":currentLang  
            };

            ClientService.getSetting(response).then(function(response){
                debugger;
                if(response.error) return;
                vm.quizSetting = response.data;
                vm.quiztime = vm.quizSetting.quiz_delay_hrs;
                console.log(vm.quizSetting);  
        ...
        function startQuiz() {
            ...
            ClientService.startQuiz().then(function(response){
                debugger;
                vm.quiz = response.quiz_questions[0];
                ...
                vm.quizResult.currentTime = $filter('date')(new Date(), 'medium');
        ...
        function dbTest() {
            sendEmail('sss');
            // alert(vm.quizClient.daily_percentage_discount);
            // getSetting();
            // alert(localStorage.getItem('NG_TRANSLATE_LANG_KEY') ? localStorage.getItem('NG_TRANSLATE_LANG_KEY') : $rootScope.language);
